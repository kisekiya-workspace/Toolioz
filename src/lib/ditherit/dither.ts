...
export function ditherImage(
  ...
  if (params.algorithm === "ordered") {
    for (let oy = 0; oy < outputHeight; oy += step) for (let ox = 0; ox < outputWidth; ox += step) {
      const px = clamp(Math.floor(ox * sx), 0, iw - 1);
      const py = clamp(Math.floor(oy * sy), 0, ih - 1);
      if (src[(py * iw + px) * 4 + 3] < 12) continue; // Skip erased background

      const v = gray[py * iw + px];
...
// Original logic by Prasanjit Dey (https://github.com/prasanjit-dey-ux/ditherit)
...
export function dotsToSVG(dots: DotCoord[], w: number, h: number, dotColor: string, bgColor: string): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}">\n  <!-- ditherit by Prasanjit Dey (https://github.com/prasanjit-dey-ux/ditherit) -->\n  <rect width="${w}" height="${h}" fill="${bgColor}"/>\n${circles}\n</svg>`;
}

import type { AsciiParams } from "./ascii";

export function generateReactCode(p: DitherParams, ap: AsciiParams | Record<string, unknown>, mode: string, videoRender: string, removeBackground = false, _detectedType?: string): string {
  ...
  return `// ditherit component export - original creator: https://github.com/prasanjit-dey-ux/ditherit
