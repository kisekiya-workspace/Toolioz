export const calculatorPageStyles = {
  ...
  inputCard: 'rounded-2xl border border-zinc-200 bg-white p-6 shadow-xs sm:p-10 dark:border-zinc-800 dark:bg-zinc-900',
  ...
  resultCard:
    'rounded-2xl border border-white/10 p-6 text-center text-white shadow-xl sm:p-8 relative overflow-hidden',
  resultLabel: 'mb-2 text-xs font-bold uppercase tracking-wider text-white/90',
  resultValue: 'mb-6 break-words font-mono text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl text-white drop-shadow-xs',
  ...
  btn: '!w-full !rounded-xl !bg-white !py-3 !font-bold !text-zinc-950 !border-0 hover:!bg-zinc-100 !shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer',
  infoBox:
  ...
  mobileResultBar:
    'fixed bottom-0 left-0 z-50 hidden w-full items-center justify-between border-t border-zinc-200 bg-white px-6 py-4 shadow-lg dark:border-zinc-800 dark:bg-zinc-900 max-[900px]:flex',
  mobileInfo: 'min-w-0',
  mobileLabel: 'text-xs font-semibold text-zinc-500',
  mobileValue: 'text-xl font-extrabold text-blue-600',
  mobileAction:
    'rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-xs hover:bg-blue-700',
} as const;
