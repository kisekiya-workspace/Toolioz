...
const presets: Record<PercentageToolKind, [number, number, number]> = { percentage: [18, 250, 0], change: [80, 100, 0], reverse: [80, 20, 0], discount: [1000, 20, 5] };
...
export default function PercentageCalculatorClient({ kind }: { kind: PercentageToolKind }) {
  ...
  }, [a, b, c, basicMode, kind, reverseMode]);

  const reset = () => { setA(presets[kind][0]); setB(presets[kind][1]); setC(presets[kind][2]); setBasicMode('of'); setReverseMode('decrease'); };
  const fieldLabels = kind === 'percentage' ? (basicMode === 'of' ? ['Percentage', 'Number'] : ['Part (X)', 'Whole (Y)']) : kind === 'change' ? ['Original value', 'New value'] : kind === 'reverse' ? ['Final value', 'Percentage rate'] : ['Original price', 'Discount rate'];
